...
final class WaypointViewModel {
    ...
    func addWaypoint(name: String, coordinate: CLLocationCoordinate2D, category: String?) {
        ...
        context.insert(wp)
        do { try context.save() } catch { logger.error("Failed to save waypoint data: \(error)") }
        loadWaypoints()
    ...
    func updateWaypoint(_ waypoint: Waypoint, name: String, category: String?) {
        ...
        do { try modelContext?.save() } catch { logger.error("Failed to save waypoint data: \(error)") }
        loadWaypoints()
    }
